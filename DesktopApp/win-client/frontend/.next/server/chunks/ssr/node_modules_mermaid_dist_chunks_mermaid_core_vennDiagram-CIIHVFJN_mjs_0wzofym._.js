...
`,"getStyles");function ak(a){let b=new Map;for(let c of a){let a=c.targets.join("|"),d=b.get(a);d?Object.assign(d,c.styles):b.set(a,{...c.styles})}return b}(0,f.__name)(ak,"buildStyleByKey");var al=(0,f.__name)((a,b,d,f)=>{let k=f.db,m=k.getConfig?.(),{themeVariables:n,look:o,handDrawnSeed:p}=(0,e.getConfig)(),q="handDrawn"===o,r=[n.venn1,n.venn2,n.venn3,n.venn4,n.venn5,n.venn6,n.venn7,n.venn8].filter(Boolean),s=k.getDiagramTitle?.(),t=k.getSubsetData(),u=k.getTextData(),v=ak(k.getStyleData()),w=m?.width??800,x=m?.height??450,y=w/1600,A=s?48*y:0,B=n.primaryTextColor??n.textColor,E=(0,c.selectSvgElement)(b);E.attr("viewBox",`0 0 ${w} ${x}`),s&&E.append("text").text(s).attr("class","venn-title").attr("font-size",`${32*y}px`).attr("text-anchor","middle").attr("dominant-baseline","middle").attr("x","50%").attr("y",32*y).style("fill",n.vennTitleTextColor||n.titleColor);let H=(0,g.select)(document.createElement("div")),I=(function(a={}){let b=!1,c=600,d=350,e=15,f=1e3,g=Math.PI/2,h=!0,i=null,j=!0,k=!0,l=null,m=null,n=!1,o=null,p=!!a&&!!a.symmetricalTextCentre&&a.symmetricalTextCentre,q={},r=a&&a.colourScheme?a.colourScheme:a&&a.colorScheme?a.colorScheme:["#1f77b4","#ff7f0e","#2ca02c","#d62728","#9467bd","#8c564b","#e377c2","#7f7f7f","#bcbd22","#17becf"],s=0,t=function(a){if(a in q)return q[a];var b=q[a]=r[s];return(s+=1)>=r.length&&(s=0),b},u=z,v=C;function w(q){let r=q.datum(),s=new Set;r.forEach(a=>{0==a.size&&1==a.sets.length&&s.add(a.sets[0])});let w={},x={};if((r=r.filter(a=>!a.sets.some(a=>s.has(a)))).length>0){let a=u(r,{lossFunction:v,distinct:n});h&&(a=F(a,g,m)),x=L(w=G(a,c,d,e,i),r,p)}let y={};function z(a){return a.sets in y?y[a.sets]:1==a.sets.length?""+a.sets[0]:void 0}r.forEach(a=>{a.label&&(y[a.sets]=a.label)}),q.selectAll("svg").data([w]).enter().append("svg");let A=q.select("svg");b?A.attr("viewBox",`0 0 ${c} ${d}`):A.attr("width",c).attr("height",d);let B={},C=!1;function D(a){return b=>{var e,f;return e=a.sets.map(a=>{let e=B[a],f=w[a];return e||(e={x:c/2,y:d/2,radius:1}),f||(f={x:c/2,y:d/2,radius:1}),{x:e.x*(1-b)+f.x*b,y:e.y*(1-b)+f.y*b,radius:e.radius*(1-b)+f.radius*b}}),f=o,N(M(e),f)}}A.selectAll(".venn-area path").each(function(a){let b,c=this.getAttribute("d");1==a.sets.length&&c&&!n&&(C=!0,B[a.sets[0]]={x:Number.parseFloat((b=c.split(" "))[1]),y:Number.parseFloat(b[2]),radius:-Number.parseFloat(b[4])})});let E=A.selectAll(".venn-area").data(r,a=>a.sets),H=E.enter().append("g").attr("class",a=>`venn-area venn-${1==a.sets.length?"circle":"intersection"}${a.colour||a.color?" venn-coloured":""}`).attr("data-venn-sets",a=>a.sets.join("_")),I=H.append("path"),K=H.append("text").attr("class","label").text(a=>z(a)).attr("text-anchor","middle").attr("dy",".35em").attr("x",c/2).attr("y",d/2);function O(a){return"function"==typeof a.transition?a.transition("venn").duration(f):a}k&&(I.style("fill-opacity","0").filter(a=>1==a.sets.length).style("fill",a=>a.colour?a.colour:a.color?a.color:t(a.sets)).style("fill-opacity",".25"),K.style("fill",b=>b.colour||b.color?"#FFF":a.textFill?a.textFill:1==b.sets.length?t(b.sets):"#444"));let P=q;C&&"function"==typeof P.transition?(P=O(q)).selectAll("path").attrTween("d",D):P.selectAll("path").attr("d",a=>N(M(a.sets.map(a=>w[a])),void 0),o);let Q=P.selectAll("text").filter(a=>a.sets in x).text(a=>z(a)).attr("x",a=>Math.floor(x[a.sets].x)).attr("y",a=>Math.floor(x[a.sets].y));j&&(C?"on"in Q?Q.on("end",J(w,z)):Q.each("end",J(w,z)):Q.each(J(w,z)));let R=O(E.exit()).remove();"function"==typeof E.transition&&R.selectAll("path").attrTween("d",D);let S=R.selectAll("text").attr("x",c/2).attr("y",d/2);return null!==l&&(K.style("font-size","0px"),Q.style("font-size",l),S.style("font-size","0px")),{circles:w,textCentres:x,nodes:E,enter:H,update:P,exit:R}}return w.wrap=function(a){return arguments.length?(j=a,w):j},w.useViewBox=function(){return b=!0,w},w.width=function(a){return arguments.length?(c=a,w):c},w.height=function(a){return arguments.length?(d=a,w):d},w.padding=function(a){return arguments.length?(e=a,w):e},w.distinct=function(a){return arguments.length?(n=a,w):n},w.colours=function(a){return arguments.length?(t=a,w):t},w.colors=function(a){return arguments.length?(t=a,w):t},w.fontSize=function(a){return arguments.length?(l=a,w):l},w.round=function(a){return arguments.length?(o=a,w):o},w.duration=function(a){return arguments.length?(f=a,w):f},w.layoutFunction=function(a){return arguments.length?(u=a,w):u},w.normalize=function(a){return arguments.length?(h=a,w):h},w.scaleToFit=function(a){return arguments.length?(i=a,w):i},w.styled=function(a){return arguments.length?(k=a,w):k},w.orientation=function(a){return arguments.length?(g=a,w):g},w.orientationOrder=function(a){return arguments.length?(m=a,w):m},w.lossFunction=function(a){return arguments.length?(v="default"===a?C:"logRatio"===a?D:a,w):v},w})().width(w).height(x-A);H.datum(t).call(I);let K=q?O.default.svg(H.select("svg").node()):void 0,P=function(a,b={}){let{lossFunction:c,layoutFunction:d=z,normalize:e=!0,orientation:f=Math.PI/2,orientationOrder:g,width:h=600,height:i=350,padding:j=15,scaleToFit:k=!1,symmetricalTextCentre:l=!1,distinct:m,round:n=2}=b,o=d(a,{lossFunction:"default"!==c&&c?"logRatio"===c?D:c:C,distinct:m});e&&(o=F(o,f,g));let p=G(o,h,i,j,k),q=L(p,a,l),r=new Map(Object.keys(p).map(a=>[a,{set:a,x:p[a].x,y:p[a].y,radius:p[a].radius}])),s=a.map(a=>{let b=a.sets.map(a=>r.get(a)),c=M(b),d=N(c,n);return{circles:b,arcs:c,path:d,area:a,has:new Set(a.sets)}});return s.map(({circles:a,arcs:b,path:c,area:d})=>({data:d,text:q[d.sets],circles:a,arcs:b,path:c,distinctPath:c+function(a){let b="";for(let c of s)c.has.size>a.length&&a.every(a=>c.has.has(a))&&(b+=" "+c.path);return b}(d.sets)}))}(t,{width:w,height:x-A,padding:m?.padding??15}),Q=new Map;for(let a of P){let b=am([...a.data.sets].sort());Q.set(b,a)}u.length>0&&an(m,Q,H,u,y,v);let R=(0,h.isDark)(n.background||"#f4f4f4");H.selectAll(".venn-circle").each(function(a,b){let c=(0,g.select)(this),d=am([...a.sets].sort()),e=v.get(d),f=e?.fill||r[b%r.length]||n.primaryColor;c.classed(`venn-set-${b%8}`,!0);let h=e?.["fill-opacity"]??.1,k=e?.stroke||f,m=e?.["stroke-width"]||`${5*y}`;if(q&&K){let a=Q.get(d);if(a&&a.circles.length>0){let d=a.circles[0],e=K.circle(d.x,d.y,2*d.radius,{roughness:.7,seed:p,fill:l(f,.7),fillStyle:"hachure",fillWeight:2,hachureGap:8,hachureAngle:-41+60*b,stroke:k,strokeWidth:parseFloat(String(m))});c.select("path").remove(),c.node()?.insertBefore(e,c.select("text").node())}}else c.select("path").style("fill",f).style("fill-opacity",h).style("stroke",k).style("stroke-width",m).style("stroke-opacity",.95);let o=e?.color||(R?(0,i.lighten)(f,30):(0,j.darken)(f,30));c.select("text").style("font-size",`${48*y}px`).style("fill",o)}),q&&K?H.selectAll(".venn-intersection").each(function(a){let b=(0,g.select)(this),c=am([...a.sets].sort()),d=v.get(c),e=d?.fill;if(e){let a=b.select("path"),c=a.attr("d");if(c){let b=K.path(c,{roughness:.7,seed:p,fill:l(e,.3),fillStyle:"cross-hatch",fillWeight:2,hachureGap:6,hachureAngle:60,stroke:"none"}),d=a.node();d?.parentNode?.insertBefore(b,d),a.remove()}}else b.select("path").style("fill-opacity",0);b.select("text").style("font-size",`${48*y}px`).style("fill",d?.color??n.vennSetTextColor??B)}):(H.selectAll(".venn-intersection text").style("font-size",`${48*y}px`).style("fill",a=>{let b=am([...a.sets].sort());return v.get(b)?.color??n.vennSetTextColor??B}),H.selectAll(".venn-intersection path").style("fill-opacity",a=>{let b=am([...a.sets].sort());return+!!v.get(b)?.fill}).style("fill",a=>{let b=am([...a.sets].sort());return v.get(b)?.fill??"transparent"}));let S=E.append("g").attr("transform",`translate(0, ${A})`),T=H.select("svg").node();if(T&&"childNodes"in T)for(let a of[...T.childNodes])S.node()?.appendChild(a);(0,e.configureSvgSize)(E,x,w,m?.useMaxWidth??!0)},"draw");function am(a){return a.join("|")}function an(a,b,c,d,e,f){let g=a?.useDebugLayout??!1,h=c.select("svg").append("g").attr("class","venn-text-nodes"),i=new Map;for(let a of d){let b=am(a.sets),c=i.get(b);c?c.push(a):i.set(b,[a])}for(let[a,c]of i.entries()){let d=b.get(a);if(!d?.text)continue;let i=d.text.x,j=d.text.y,k=Math.min(...d.circles.map(a=>a.radius)),l=Math.min(...d.circles.map(a=>a.radius-Math.hypot(i-a.x,j-a.y))),m=Number.isFinite(l)?Math.max(0,l):0;0===m&&Number.isFinite(k)&&(m=.6*k);let n=h.append("g").attr("class","venn-text-area").attr("font-size",`${40*e}px`);g&&n.append("circle").attr("class","venn-text-debug-circle").attr("cx",i).attr("cy",j).attr("r",m).attr("fill","none").attr("stroke","purple").attr("stroke-width",1.5*e).attr("stroke-dasharray",`${6*e} ${4*e}`);let o=Math.max(80*e,2*m*.95),p=Math.max(60*e,2*m*.95),q=(d.data.label&&d.data.label.length>0?Math.min(32*e,.25*m):0)+(c.length<=2?30*e:0),r=i-o/2,s=j-p/2+q,t=Math.max(1,Math.ceil(Math.sqrt(c.length))),u=Math.max(1,Math.ceil(c.length/t)),v=o/t,w=p/u;for(let[a,b]of c.entries()){let c=a%t,d=Math.floor(a/t),h=r+v*(c+.5),i=s+w*(d+.5);g&&n.append("rect").attr("class","venn-text-debug-cell").attr("x",r+v*c).attr("y",s+w*d).attr("width",v).attr("height",w).attr("fill","none").attr("stroke","teal").attr("stroke-width",+e).attr("stroke-dasharray",`${4*e} ${3*e}`);let j=.9*v,k=.9*w,l=n.append("foreignObject").attr("class","venn-text-node-fo").attr("width",j).attr("height",k).attr("x",h-j/2).attr("y",i-k/2).attr("overflow","visible"),m=f.get(b.id)?.color,o=l.append("xhtml:span").attr("class","venn-text-node").style("display","flex").style("width","100%").style("height","100%").style("white-space","normal").style("align-items","center").style("justify-content","center").style("text-align","center").style("overflow-wrap","normal").style("word-break","normal").text(b.label??b.id);m&&o.style("color",m)}}}(0,f.__name)(am,"stableSetsKey"),(0,f.__name)(an,"renderTextNodes"),a.s(["diagram",0,{parser:P,db:ai,renderer:{draw:al},styles:aj}],599033)}];

//# sourceMappingURL=node_modules_mermaid_dist_chunks_mermaid_core_vennDiagram-CIIHVFJN_mjs_0wzofym._.js.map