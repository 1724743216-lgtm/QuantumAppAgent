...
// EvoScientist's paths.py resolves to, by default `~/.tyqa/memories`.
//
 ...
 *  DATA_DIR), relocatable via TYQA_DATA_DIR. Mirrors skills.ts so both
...
function globalDataDir(): string {
  const env = process.env.TYQA_DATA_DIR;
  if (env && env.trim()) return expandHome(env);
  return join(homedir(), ".tyqa");
}
...
export function memoryDirPath(): string {
  const env =
    process.env.TYQA_MEMORIES_DIR ||
    process.env.TYQA_MEMORY_DIR;
  if (env && env.trim()) return expandHome(env);
