...
export async function getWorkspaceDir(): Promise<string> {
  ...
  workspace ||= process.env.TYQA_WORKSPACE_DIR;
  if (!workspace) {
    throw new Error(
      "No active TYQA workspace found. Start the backend with `tyqa deploy` first."
    );
  }
  ...
  if (!stat.isDirectory()) {
    throw new Error("The active TYQA workspace is not a directory.");
  }
