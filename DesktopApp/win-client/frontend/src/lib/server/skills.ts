...
import { getWorkspaceDir } from "@/lib/server/workspace";
...
function envPath(key: string): string | undefined {
  ...
  return value && value.trim() ? expandHomePath(value.trim()) : undefined;
}

/** TYQA's global data dir: `~/.tyqa` by default (paths.py DATA_DIR),
 *  relocatable via TYQA_DATA_DIR, exactly like the backend. */
function globalDataDir(): string {
  return envPath("TYQA_DATA_DIR") ?? join(homedir(), ".tyqa");
}

/** Where skills install: tyqa's GLOBAL skills tier
 *  (`DATA_DIR/skills` = `~/.tyqa/skills`), matching `install_skill()`'s
...
export interface SkillDir {
  dir: string;
  source: SkillSource;
  /** True only for user-manageable tiers. Built-in backend skills are read-only. */
  writable: boolean;
}
...
async function addSkillDir(
  ...
  if (requireExists && !(await isDirectory(resolved))) return;
  let real = resolved;
  try {
  ...
  seen.add(key);
  dirs.push({ dir: real, source, writable });
}

async function workspaceSkillsDir(): Promise<string | undefined> {
  const explicit = envPath("TYQA_SKILLS_DIR");
  if (explicit) return explicit;
  try {
    return join(await getWorkspaceDir(), "skills");
  } catch {
    const workspace = envPath("TYQA_WORKSPACE_DIR");
    return workspace ? join(workspace, "skills") : undefined;
  }
}

function builtinSkillDirCandidates(): string[] {
  const candidates: string[] = [];
  const explicit = envPath("TYQA_BUILTIN_SKILLS_DIR");
  if (explicit) candidates.push(explicit);

  for (const key of ["TYQA_BACKEND_DIR", "TYQA_ROOT_DIR"]) {
    const root = envPath(key);
    if (root) candidates.push(join(root, "tyqa", "skills"));
  }

  const cwd = process.cwd();
  const parent = resolve(cwd, "..");
  candidates.push(
    join(parent, "QuantumAppAgent", "tyqa", "skills"),
    join(parent, "EvoScientist", "tyqa", "skills"),
    join(cwd, "tyqa", "skills")
  );
  return candidates;
}

async function builtinSkillsDir(): Promise<string | undefined> {
  for (const candidate of builtinSkillDirCandidates()) {
    if (await isDirectory(candidate)) return candidate;
  }
  return undefined;
}

/** Skill tiers mirrored from TYQA backend priority:
 *  workspace/skills > ~/.tyqa/skills > backend tyqa/skills. */
export async function getSkillDirs({
  includeBuiltin = true,
  writableOnly = false,
}: {
  ...
  await addSkillDir(dirs, seen, await workspaceSkillsDir(), "workspace", true);
  await addSkillDir(dirs, seen, SKILLS_INSTALL_DIR, "global", true);
  if (includeBuiltin) {
    await addSkillDir(
      dirs,
      seen,
      await builtinSkillsDir(),
      "builtin",
      false,
      true
    );
  }
  return writableOnly ? dirs.filter((dir) => dir.writable) : dirs;
}
...
async function readInstalledVersion(name: string): Promise<string | undefined> {
  for (const { dir } of await getSkillDirs()) {
    try {
...
export async function getSkillDetail(name: string): Promise<SkillDetail> {
  ...
  for (const skillDir of await getSkillDirs()) {
    const { dir } = skillDir;
    try {
