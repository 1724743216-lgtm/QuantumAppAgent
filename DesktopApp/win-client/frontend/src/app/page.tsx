...
function HomePageInner({
  ...
  const sidebarToggleLabel = view
    ...
    : sidebar
    ? t("header.hideResearch")
    : t("header.showResearch");
  const inspectorToggleLabel = inspector
    ? t("header.hideInspector")
    : t("header.showWorkspace");
        ...
        <div className="relative flex-1 overflow-hidden">
          {sidebar && isDesktopLayout === false && (
            <div className="absolute inset-0 z-40 flex md:hidden">
              <button
                type="button"
                aria-label={t("header.closeResearch")}
                className="absolute inset-0 bg-black/40"
                onClick={closeSidebar}
              />
              <aside
                aria-label={view ? t("header.navigation") : t("header.researchNavigation")}
          ...
          {inspector && isDesktopLayout === false && (
            <div className="absolute inset-0 z-40 flex justify-end md:hidden">
              <button
                type="button"
                aria-label={t("header.hideInspector")}
                className="absolute inset-0 bg-black/40"
        ...
        <div className="flex h-screen items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-2 text-muted-foreground">
              {t("home.configureDeployment")}
            </p>
            <Button
              onClick={() => setConfigDialogOpen(true)}
              className="mt-4"
            >
              {t("home.openConfiguration")}
            </Button>
          </div>
        </div>
      </>
