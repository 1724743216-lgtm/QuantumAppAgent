...
const SUGGESTED_PROMPT_KEYS = [
  "chat.suggest.recentPapers",
  "chat.suggest.experimentPlan",
  "chat.suggest.analyzeFiles",
] as const;
  ...
  ({
    ...
    useEffect(() => {
      ...
      for (const t of liveAgentTasks) {
        ...
        const translatedAgentLabel =
          t.agent_name === "writing"
            ? tRef.current("agents.writingAgent")
            : t.agent_name === "data_analysis"
            ? tRef.current("agents.dataAnalysisAgent")
            : agentLabel(t.agent_name);
        toast.success(
          tRef.current("chat.autoReported", { agent: translatedAgentLabel })
        );
      ...
      async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
          ...
          if (!response.ok || !data.files) {
            throw new Error(data.error || tRef.current("chat.uploadFailed"));
          }
          setPendingFiles((currentFiles) => [...currentFiles, ...data.files!]);
          toast.success(
            tRef.current(
              data.files.length === 1
                ? "chat.uploadedFileSingular"
                : "chat.uploadedFilePlural",
          ...
          toast.error(
            error instanceof Error
              ? error.message
              : tRef.current("chat.uploadFailed")
          );
        } finally {
      ...
      async (cmd: ReturnType<typeof parseModelCommand>) => {
        ...
        try {
          if (cmd.kind === "reset") {
            ...
            toast.success(tRef.current("chat.modelOverrideCleared"));
          } else {
            ...
            toast.success(
              tRef.current(
                cmd.provider ? "chat.modelSetProvider" : "chat.modelSet",
                { model: cmd.model, provider: cmd.provider ?? "" }
              )
            );
          }
        } catch (err) {
          toast.error(
            err instanceof Error
              ? tRef.current("chat.modelUpdateFailedWithError", {
                  error: err.message,
                })
              : tRef.current("chat.modelUpdateFailed")
          );
        }
      },
      [setModelOverride]
    ...
    const steerQueuedMessage = useCallback((id: number) => {
      ...
      toast.info(tRef.current("chat.steerQueuedToast"));
    }, []);
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>{translate("chat.autoApproveDialogTitle")}</DialogTitle>
              <DialogDescription>
                {translate("chat.autoApproveDialogDescription")}
              </DialogDescription>
            </DialogHeader>
            <div className="flex items-start gap-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-950 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-100">
              <TriangleAlert
                ...
                aria-hidden="true"
              />
              <p>{translate("chat.autoApproveScopeWarning")}</p>
            </div>
            <DialogFooter>
              <Button
                variant="outline"
              ...
              >
                {translate("chat.enableAutoApprove")}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>{translate("chat.modelPickerTitle")}</DialogTitle>
              <DialogDescription>
                {translate("chat.modelPickerDescriptionPrefix")}{" "}
                <span className="font-mono text-xs">/model &lt;name&gt;</span>{" "}
                {translate("chat.modelPickerDescriptionSuffix")}
              </DialogDescription>
            </DialogHeader>
            <div className="mt-2 space-y-2">
              <div className="text-xs text-muted-foreground">
                {translate("chat.current")}: {" "}
                <span className="font-mono">
                  {currentModel
                    ...
                    : translate("chat.deploymentDefault")}
                </span>
                {modelRegistryLoading && (
                  <span className="ml-2 italic">
                    {translate("chat.loadingRegistry")}
                  </span>
                )}
              </div>
              {isFallbackModelList && (
                <div className="border-[var(--color-warning)]/30 bg-[var(--color-warning)]/5 flex items-start gap-2 rounded-md border px-3 py-2 text-xs text-foreground">
                  ...
                  <span>
                    {translate("chat.modelFallbackPrefix")}{" "}
                    <span className="mx-1 font-mono">/api/models</span>{" "}
                    {translate("chat.modelFallbackMiddle", {
                      error: modelRegistryError ? ` (${modelRegistryError})` : "",
                    })}{" "}
                    <span className="font-mono">/model &lt;name&gt;</span>
                    {translate("chat.modelFallbackSuffix")}
                  </span>
                </div>
              )}
              <input
                ...
                placeholder={translate("chat.modelFilterPlaceholder")}
                autoFocus
              ...
              <ul className="max-h-72 space-y-1 overflow-y-auto pl-0">
                {filteredPickerModels.length === 0 && (
                  <li className="px-3 py-2 text-xs text-muted-foreground">
                    {translate("chat.noModelsMatch", { query: modelSearch })}
                  </li>
                )}
                      ...
                      <button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            toast.error(
                              err instanceof Error
                                ? translate("chat.modelUpdateFailedWithError", {
                                    error: err.message,
                                  })
                                : translate("chat.modelUpdateFailed")
                            );
                          }
                        }}
                        className={cn(
            ...
            <DialogFooter>
              <Button
                ...
                onClick={async () => {
                  try {
                    await setModelOverride(null);
                    toast.success(translate("chat.modelOverrideCleared"));
                    setModelPickerOpen(false);
                  } catch (err) {
                    toast.error(
                      err instanceof Error
                        ? translate("chat.modelClearFailedWithError", {
                            error: err.message,
                          })
                        : translate("chat.modelClearFailed")
                    );
                  }
                }}
                disabled={!modelOverride}
              >
                {translate("chat.resetToDefault")}
              </Button>
          ...
          >
            {isThreadLoading ? (
              <div className="flex items-center justify-center p-8">
                <p className="text-muted-foreground">{translate("home.loading")}</p>
              </div>
            ) : (
              <>
                {processedMessages.length === 0 && !isLoading && (
                  <div className="flex min-h-[42vh] flex-col items-center justify-center px-3 pt-12 text-center sm:pt-16">
                    <h2 className="text-pretty text-lg font-semibold sm:text-xl">
                      {translate("chat.emptyTitle")}
                    </h2>
                    <p className="mt-2 max-w-lg text-sm text-muted-foreground">
                      {translate("chat.emptyDescription")}
                    </p>
                    <div className="mt-4 flex max-w-2xl flex-wrap justify-center gap-2">
                      {SUGGESTED_PROMPT_KEYS.map((promptKey) => {
                        const prompt = translate(promptKey);
                        return (
                          <button
                            key={promptKey}
                            type="button"
        ...
        <div className="flex-shrink-0 bg-background">
          {queuedMessages.length > 0 && (
            <div
              aria-label={translate("chat.queuedMessagesAria")}
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        aria-label={translate("chat.reorderQueuedMessage", {
                          index: index + 1,
                          count: queuedMessages.length,
                        })}
                        title={translate("chat.reorderQueuedTitle")}
                      ...
                      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                        ...
                        {q.files.length > 0 && (
                          <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                            ...
                            {translate(
                              q.files.length === 1
                                ? "chat.fileCountSingular"
                                : "chat.fileCountPlural",
                      ...
                      <div className="flex shrink-0 items-center gap-0.5">
                        <button
                          ...
                          aria-label={translate("chat.steerQueuedAria")}
                          title={translate("chat.steerQueuedTitle")}
                        ...
                        >
                          ...
                          <span className="hidden sm:inline">
                            {translate("chat.steer")}
                          </span>
                        </button>
                        <button
                          type="button"
                          onClick={() => removeQueuedMessage(q.id)}
                          aria-label={translate("chat.removeQueuedAria")}
                          title={translate("chat.removeQueuedTitle")}
                        ...
                        >
                          <summary
                            aria-label={translate("chat.moreQueuedActionsAria")}
                            title={translate("chat.moreActions")}
          ...
          >
                            ...
                            {(() => {
                              if (isCompleted) {
                                return [
                                  ...
                                  >
                                    {translate("chat.allTasksCompleted")}
                                  </span>,
                                ];
                              }
                ...
                {metaOpen && (
                  <>
                    <div className="sticky top-0 flex items-stretch bg-sidebar text-sm">
                      ...
                      <button
                        aria-label={translate("chat.close")}
                        className="flex-1"
                          ...
                          .map(([status, todos]) => (
                            ...
                            >
                              <h3 className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-tertiary">
                                {
                                  {
                                    pending: translate("tasksFiles.pending"),
                                    in_progress: translate("tasksFiles.inProgress"),
                                    completed: translate("tasksFiles.completed"),
                                  }[status]
                                }
                              </h3>
              ...
              >
                <span className="flex items-center gap-1.5">
                  <TriangleAlert
                    ...
                    aria-hidden="true"
                  />
                  {translate("chat.autoApproveBanner")}
                </span>
                <button
                  type="button"
              ...
              <div className="flex items-center gap-1.5 border-t border-border px-3 py-1.5 text-xs text-muted-foreground">
                {currentModel && (
                  <button
                    ...
                    title={translate("chat.changeModelTitle")}
                    aria-label={translate("chat.changeModelAria")}
                ...
                {runningAgents > 0 && (
                  <button
                    ...
                    aria-label={translate(
                      runningAgents === 1
                        ? "chat.runningAgentAriaSingular"
                        : "chat.runningAgentAriaPlural",
                      { count: runningAgents }
                    )}
                  ...
                  >
                    ...
                    {translate(
                      runningAgents === 1
                        ? "chat.agentCountSingular"
                        : "chat.agentCountPlural",
                      { count: runningAgents }
                    )}
                  </button>
                )}
            ...
            >
              {pendingFiles.length > 0 && (
                <div
                  aria-label={translate("chat.attachedFiles")}
                  ...
                  {pendingFiles.map((file) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        aria-label={translate("chat.removePendingFile", {
                          name: file.name,
                        })}
                        title={translate("chat.removePendingFile", {
                          name: file.name,
                        })}
              ...
              <textarea
                ...
                aria-label={translate("chat.messageAria")}
                disabled={hasPendingInterrupt}
                placeholder={
                  hasPendingInterrupt
                    ? translate("chat.respondPlaceholder")
                    : isLoading
                    ? translate("chat.queuePlaceholder")
                    : translate("chat.askPlaceholder")
                }
              ...
              <div className="flex items-center justify-between gap-2 p-2 sm:p-2.5">
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    aria-label={translate("chat.uploadAria")}
                    title={translate("chat.uploadTitle")}
                  ...
                  <button
                    ...
                    title={translate("chat.autoApproveTitle")}
                    className={cn(
                  ...
                  >
                    ...
                    <span className="hidden min-[360px]:inline">
                      {autoApprove
                        ? translate("chat.autoApproveOn")
                        : translate("chat.autoApprove")}
                    </span>
                  </button>
                ...
                <div className="flex justify-end gap-2">
                  <Button
                    ...
                    aria-label={
                      isLoading
                        ? translate("chat.stopGenerating")
                        : translate("chat.sendMessage")
                    }
                  >
                    {isLoading ? (
                      <>
