...
function NodeDetailPanel({
  ...
  useEffect(() => {
    ...
    fetch(`/api/memory?path=${encodeURIComponent(node.path)}`, {
      ...
      .then(async (res) => {
        ...
        if (!res.ok) {
          throw new Error(
            data.error || tRef.current("observationGraph.loadObservationFailed")
          );
        }
      ...
      .catch((error) => {
        ...
        setContentError(
          error instanceof Error
            ? error.message
            : tRef.current("observationGraph.loadObservationFailed")
        );
      })
      .finally(() => {
  ...
  const getRelationLabel = (relation: string) => {
    switch (relation) {
      case "complements":
        return translate("observationGraph.relation.complements");
      case "contradicts":
        return translate("observationGraph.relation.contradicts");
      case "supersedes":
        return translate("observationGraph.relation.supersedes");
      default:
    ...
    <div className="flex h-full flex-col overflow-hidden border-l border-border bg-background shadow-xl md:shadow-none">
      ...
      <div className="flex items-start justify-between gap-2 border-b border-border px-4 py-3">
        ...
        <button
          ...
          aria-label={translate("observationGraph.closeDetails")}
          title={translate("observationGraph.closeDetails")}
        >
          <X
            className="h-4 w-4"
            aria-hidden="true"
      ...
      >
        ...
        {relatedEdges.length > 0 && (
          <div className="border-b border-border px-4 py-3">
            <p className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">
              {translate("observationGraph.relations")}
            </p>
        ...
        <div className="px-4 py-3">
          {loadingContent ? (
            ...
            <p className="text-xs text-muted-foreground">
              {translate("observationGraph.noBody")}
            </p>
          )}
        </div>
      </div>
    ...
    <>
      ...
      <div className="absolute left-3 top-3 z-10 flex gap-1">
        <button
          ...
          aria-label={translate("observationGraph.zoomInAria")}
          title={translate("observationGraph.zoomIn")}
        >
          <ZoomIn
        ...
        <button
          ...
          className={GRAPH_BUTTON_CLASS}
          aria-label={translate("observationGraph.zoomOutAria")}
          title={translate("observationGraph.zoomOut")}
        >
          <ZoomOut
            className="h-3.5 w-3.5"
            aria-hidden="true"
        ...
        <button
          ...
          onClick={resetView}
          className={GRAPH_BUTTON_CLASS}
          aria-label={translate("observationGraph.resetAria")}
          title={translate("observationGraph.reset")}
        >
          <Maximize2
            className="h-3.5 w-3.5"
            aria-hidden="true"
      ...
      <div className="absolute right-3 top-3 z-10 flex max-w-[calc(100%-1.5rem)] select-none flex-col gap-1 rounded border border-border bg-background/90 px-2.5 py-2 text-[11px] shadow-sm backdrop-blur-sm">
        {(["complements", "contradicts", "supersedes"] as const).map((r) => {
          ...
          return (
            <button
              ...
              title={translate(
                active
                  ? "observationGraph.hideEdges"
                  : "observationGraph.showEdges",
                { relation: getRelationLabel(r) }
              )}
        ...
        <div className="mt-1 flex flex-col gap-0.5 border-t border-border pt-1">
          {(
            [
              ["semantic", translate("observationGraph.semantic")],
              ["procedural", translate("observationGraph.procedural")],
            ] as const
          ).map(([type, label]) => {
            ...
            return (
              <button
                ...
                title={translate(
                  active
                    ? "observationGraph.hideNodes"
                    : "observationGraph.showNodes",
      ...
      <svg
        ...
        role="img"
        aria-label={translate("observationGraph.graphAria")}
        onWheel={handleWheel}
          ...
          {data.nodes.map((node) => {
            ...
            return (
              <g
                ...
                role="button"
                tabIndex={0}
                aria-label={translate("observationGraph.selectObservation", {
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col">
      ...
      <div className="flex items-center border-b border-border px-4 py-2">
        <span className="text-xs text-muted-foreground">
          {data
            ? translate("observationGraph.stats", {
      ...
      <div className="relative flex min-h-0 flex-1">
        ...
        >
          {loading && (
            <EmptyState message={translate("observationGraph.loading")} />
          )}
          {!loading && error && (
            <EmptyState
              message={translate("observationGraph.error", { error })}
            />
          )}
          {!loading && !error && data && data.nodes.length === 0 && (
            <EmptyState message={translate("observationGraph.empty")} />
          )}
          {!loading &&
            !error &&
            data &&
