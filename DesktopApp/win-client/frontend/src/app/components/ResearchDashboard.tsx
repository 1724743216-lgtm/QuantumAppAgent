...
export function ResearchDashboard({
    ...
    (async () => {
      try {
        const [obsRes, execRes, wsRes, scheduled] = await Promise.all([
          ...
          listScheduledTasks(t("schedule.unnamedTask")).catch(() => []),
        ]);
  ...
  const stats: Array<{
    ...
    {
      value: obsCount,
      label: t("researchDashboard.knowledge"),
      hint: t("researchDashboard.knowledgeHint"),
      Icon: Network,
      target: { view: "memory", tab: "knowledge" },
    },
    {
      value: runCount + obsCount,
      label: t("researchDashboard.timeline"),
      hint: t("researchDashboard.timelineHint"),
      Icon: Activity,
    ...
    },
    {
      value: wsSize.num,
      unit: wsSize.unit,
      label: t("researchDashboard.workspace"),
      hint: t("researchDashboard.workspaceHint"),
      Icon: FolderOpen,
      target: { view: "workspace" },
    },
    {
      value: scheduledCount,
      label: t("researchDashboard.scheduled"),
      hint: t("researchDashboard.scheduledHint"),
      Icon: Clock,
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          {stats.map(({ value, unit, label, hint, Icon, target }) => (
            <button
              ...
              title={hint}
              aria-label={t("researchDashboard.statAria", {
                label,
                value,
                unit: unit ? ` ${unit}` : "",
                hint,
        ...
        <div className="mt-4 sm:mt-5">
          <div className="mb-2 flex items-center gap-1.5">
            ...
            <span className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
              {t("researchDashboard.yourResearch")}
            </span>
          </div>
          <div className="space-y-1">
            {pinned.map((thread) => (
              <button
                ...
                title={t("researchDashboard.openThreadTitle", {
                  title: thread.title,
                })}
                aria-label={t("researchDashboard.openThreadAria", {
                  title: thread.title,
                })}
        ...
        <div className="mt-4 sm:mt-5">
          <div className="mb-2 flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              />
              <span className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                {t("researchDashboard.recentActivity")}
              </span>
            </div>
            <button
              type="button"
          ...
          <div className="space-y-1">
            {activity.map((item) => {
              const summary =
                item.summary ||
                (item.kind === "execution"
                  ? t("researchDashboard.executionCompleted")
                  : item.id);
              const label =
                item.label ||
                (item.kind === "execution"
                  ? t("researchDashboard.agent")
                  : t("researchDashboard.memory"));
              const kindLabel =
                item.kind === "execution"
                  ? t("researchDashboard.kind.execution")
                  : t("researchDashboard.kind.observation");
              return (
                <button
                  ...
                  title={summary}
                  aria-label={t("researchDashboard.openActivityAria", {
                    kind: kindLabel,
                    summary,
                  })}
                  className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left transition-colors hover:bg-accent/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
