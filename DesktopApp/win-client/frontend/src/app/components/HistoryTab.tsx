...
function StatsBar({ items }: { items: TimelineItem[] }) {
  ...
  const stats = [
    { n: projectCount, label: t("memory.history.projects") },
    { n: items.length, label: t("memory.history.total") },
    { n: runsToday, label: t("memory.history.runsToday") },
    { n: obsToday, label: t("memory.history.obsToday") },
  ];
...
function EntryCard({
  ...
  const label = t(
    open ? "memory.history.collapseExecution" : "memory.history.expandExecution",
    { agent: entry.agent, summary: entry.summary }
  );
              ...
              {entry.obs_ids.length > 0 && (
                <div className="border-t border-border px-3 py-2.5">
                  <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                    {t("memory.history.linkedObservations")}
                  </p>
                  <div className="flex flex-wrap gap-1.5">
                    {entry.obs_ids.map((id) => (
                      <button
                        ...
                        title={t("memory.history.jumpToObservationTitle", { id })}
                        aria-label={t("memory.history.jumpToObservationAria", { id })}
                      >
                        {id.slice(0, 14)}...
                      </button>
                    ))}
  ...
  return (
    ...
    >
      <button
        ...
        title={t("memory.history.viewInKnowledgeGraph")}
        aria-label={t("memory.history.viewObservationAria", { summary: obs.summary })}
        ...
        <div className="p-4">
          ...
          {truncated && (
            <p className="mt-2 text-center text-xs text-muted-foreground">
              {t("memory.history.truncated", { count: items.length })}
            </p>
          )}
        </div>
      </ScrollArea>
