...
export function MemoryPanel({
  ...
  const loadObservations = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error((data as { error?: string }).error || tRef.current("memory.error.failed"));
      setObsData(data as ObsGraphData);
    } catch (e) {
      if (reqId !== obsReqRef.current) return;
      setObsError(e instanceof Error ? e.message : tRef.current("memory.error.loadFailed"));
    } finally {
  ...
  const loadExecutions = useCallback(async () => {
    ...
    try {
      ...
      if (reqId !== execReqRef.current) return;
      setExecError(e instanceof Error ? e.message : tRef.current("memory.error.loadFailed"));
    } finally {
  ...
  const TABS = [
    { id: "identity" as const, label: t("memory.tab.identity"), Icon: Fingerprint },
    { id: "knowledge" as const, label: t("memory.tab.knowledge"), Icon: Network },
    { id: "history" as const, label: t("memory.tab.history"), Icon: History },
  ];
    ...
    <div className="flex h-full flex-col overflow-hidden">
      <header className="flex flex-shrink-0 items-start justify-between gap-3 border-b border-border px-4 py-3 sm:px-5">
        <div className="min-w-0">
          <h2 className="text-xl font-semibold sm:text-2xl">{t("memory.title")}</h2>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">
            {t("memory.descriptionBefore")} {" "}
            <span className="font-medium text-[var(--brand)]">
              {t("memory.descriptionHighlight")}
            </span>{" "}
