...
const STATUS_META: Record<
  HealthStatus,
  { dot: string; labelKey: MessageKey; pulse: boolean }
> = {
  checking: {
    dot: "bg-[var(--color-warning)]",
    labelKey: "health.connecting",
    pulse: true,
  },
  online: {
    dot: "bg-[var(--color-success)]",
    labelKey: "health.connected",
    pulse: false,
  },
  offline: {
    dot: "bg-[var(--color-error)]",
    labelKey: "health.offline",
    pulse: false,
  },
};
...
export function HealthIndicator({
  ...
  const version = info?.version
    ? t("health.langgraphVersion", { version: info.version })
    : "";
  const title =
    status === "online"
      ? t("health.connectedTo", { url: deploymentUrl, version })
      : status === "offline"
      ? t("health.offlineTitle", { url: deploymentUrl })
      : t("health.checkingTitle", { url: deploymentUrl });
    ...
    <div className="flex items-center gap-1">
      ...
      {showReconnect && (
        <Button
          ...
          title={t("health.reconnectTitle", { url: suggestedUrl })}
          aria-label={t("health.reconnectAria", { url: suggestedUrl })}
