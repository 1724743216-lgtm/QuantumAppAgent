...
function lastToolName(items: GroupedActionItem[]): string | null {
  for (let i = items.length - 1; i >= 0; i--) {
    const tcs = items[i].toolCalls;
    if (tcs.length > 0) return tcs[tcs.length - 1].name || null;
  }
  return null;
}
...
export const ActionGroup = React.memo<ActionGroupProps>(function ActionGroup({
  ...
  const toolName = lastToolName(items) ?? translate("actionGroup.tool");
  const headerText = translate(
    isStreaming
      ? count === 1
        ? "actionGroup.runningSingular"
        : "actionGroup.runningPlural"
      : count === 1
      ? "actionGroup.summarySingular"
      : "actionGroup.summaryPlural",
    { count, tool: toolName }
  );
    ...
    <div className="my-2">
      <button
        ...
        aria-label={translate(
          open ? "actionGroup.collapseAria" : "actionGroup.expandAria",
          { label: headerText }
        )}
        title={headerText}
        onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <div className="mt-2 space-y-2 border-l-2 border-border pl-3">
          ...
          <button
            ...
            aria-label={translate(
              count === 1
                ? "actionGroup.collapseCountSingular"
                : "actionGroup.collapseCountPlural",
              { count }
            )}
          >
            <ChevronUp
              ...
              className="size-3.5"
            />
            {translate("actionGroup.collapse")}
          </button>
        </div>
      )}
    </div>
