...
async function listDir(path: string, fallbackError: string): Promise<WorkspaceEntry[]> {
...
async function listAll(fallbackError: string): Promise<{
...
const CATEGORIES = [
  {
    key: "docs",
    labelKey: "workspace.category.docs",
    Icon: FileText,
    exts: [
      "pdf",
      "tex",
  ...
  {
    key: "figures",
    labelKey: "workspace.category.figures",
    Icon: ImageIcon,
    exts: [
      "png",
      "jpg",
  ...
  {
    key: "data",
    labelKey: "workspace.category.data",
    Icon: Database,
    exts: [
      "json",
      "jsonl",
  ...
  {
    key: "code",
    labelKey: "workspace.category.code",
    Icon: Code2,
    exts: [
      "py",
      "ipynb",
...
const OTHER = { key: "other", labelKey: "workspace.category.other", Icon: FileIcon } as const;
...
export function WorkspacePanel() {
  ...
  const loadDir = useCallback(async (path: string) => {
    ...
    try {
      const entries = await listDir(path, t("workspace.error.list"));
      setChildren((prev) => ({ ...prev, [path]: entries }));
      if (path === "") setError(null);
      return entries;
    } catch (err) {
      if (path === "") {
        setError(err instanceof Error ? err.message : t("workspace.error.genericLoad"));
      }
      throw err;
    } finally {
      setLoading((prev) => {
  ...
  const loadAll = useCallback(async () => {
    setTypeLoading(true);
    try {
      const { entries, truncated } = await listAll(t("workspace.error.load"));
      setAllFiles(entries);
      ...
      <div className="flex items-center justify-between gap-2 pb-1.5">
        ...
        <div className="flex items-center gap-0.5 rounded-md bg-muted p-0.5 text-xs">
          {(["tree", "type"] as const).map((m) => (
            <button
              ...
              aria-pressed={view === m}
            >
              {m === "tree" ? t("workspace.tree") : t("workspace.byType")}
            </button>
          ))}
        </div>
        <div className="flex items-center gap-0.5">
          <a
            ...
            title={t("workspace.downloadAllTitle")}
          >
          ...
          <button
            ...
            aria-label={t("workspace.refreshAria")}
