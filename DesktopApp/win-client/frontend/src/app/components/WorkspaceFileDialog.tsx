...
export const WorkspaceFileDialog = React.memo<{
    ...
    fetch(workspaceFileUrl(path))
      .then(async (res) => {
        if (!res.ok) {
          ...
          throw new Error(body?.error || t("workspace.file.error.loadWithStatus", { status: res.status }));
        }
        return res.text();
      })
      .then((text) => {
      ...
      .catch((err) => {
        if (!cancelled) setError(err.message ?? t("workspace.file.error.load"));
      })
      .finally(() => {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data?.error || t("workspace.file.error.save"));
      ...
      if (mountedRef.current) {
        setActionError(e instanceof Error ? e.message : t("workspace.file.error.save"));
      }
    } finally {
  ...
  const remove = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(data?.error || t("workspace.file.error.delete"));
      }
      onChanged?.();
      onClose();
    } catch (e) {
      if (mountedRef.current) {
        ...
        setDeleteOpen(false);
        setActionError(e instanceof Error ? e.message : t("workspace.file.error.delete"));
      }
    } finally {
    ...
    <>
              ...
              {editing ? (
                <>
                  ...
                  >
                    ...
                    {saving ? t("workspace.file.saving") : t("workspace.file.save")}
                  </Button>
                </>
              ) : (
                <>
                  {editable && (
                    <Button
                      ...
                      aria-label={t("workspace.file.editAria")}
                    >
                      <Pencil
                        size={16}
                        className="mr-1"
                  ...
                  <Button
                    ...
                    disabled={deleting}
                    aria-label={t("workspace.file.deleteAria")}
                    title={t("workspace.file.deleteAria")}
                  >
                    {deleting ? (
                      <Loader2
                        size={16}
          ...
          <div className="min-h-0 flex-1 overflow-hidden">
            {editing ? (
              <textarea
                ...
                spellCheck={false}
                aria-label={t("workspace.file.contentAria")}
                ...
                placeholder={t("workspace.file.emptyPlaceholder")}
              />
            ) : kind === "image" ? (
              ...
              <div className="flex h-full flex-col items-center justify-center gap-3 p-12 text-center">
                <p className="text-sm text-muted-foreground">
                  {tooBigForText
                    ? t("workspace.file.tooLarge")
                    : t("workspace.file.previewUnsupported")}
                </p>
                <Button
                  variant="outline"
                  size="sm"
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("workspace.file.discardTitle")}</DialogTitle>
            <DialogDescription>
              {t("workspace.file.discardDescription", { name })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            >
              {t("workspace.file.keepEditing")}
            </Button>
            <Button
              onClick={confirmDiscardChanges}
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
            >
              {t("workspace.file.discard")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("workspace.file.deleteTitle")}</DialogTitle>
            <DialogDescription>
              {t("workspace.file.deleteDescription", { name })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            <Button
              ...
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
            >
              {deleting ? t("workspace.file.deleting") : t("workspace.file.delete")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
