...
Usage: tyqa-webui [--port <port>]

Starts the TYQA Web UI and opens it in your browser.
Then enter your TYQA deployment URL (default http://127.0.0.1:6174)
from \`tyqa deploy\` to connect.
...
function poll() {
  fetch(url)
    .then(() => {
      console.log(`  ✓ TYQA Web UI is running → ${url}`);
      openBrowser();
    })
    .catch(() => {
      waited += 500;
